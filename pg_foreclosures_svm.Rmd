...
formula <- as.formula(paste("foreclosure_pc_2020 ~", paste(independent_variables, collapse = " + ")))

# Define and train the neural network model
model <- neuralnet(
  formula = formula,
  data = train_data,  # Assuming train_data is your training dataset
  hidden = c(10, 5),  # Number of neurons in each hidden layer
  linear.output = FALSE,  # Use nonlinear activation functions
  act.fct = "logistic"   # Activation function for hidden layers
)

# Visualize the network architecture
network <- model$weights[[1]]  # Extract network weights
graph <- graph_from_data_frame(network, directed = TRUE)  # Create a directed graph
plot(graph, edge.arrow.size = 0.5, vertex.size = 30, vertex.label.cex = 1.5)  # Plot the graph
